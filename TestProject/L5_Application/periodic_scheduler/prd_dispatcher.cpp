...
void period_task_400Hz(void *p)  { uint32_t count = 0; while (xSemaphoreTake(sems[prd_400Hz],  portMAX_DELAY)) period_400Hz(++count);  }
    ...
    mKHz(kHz_enabled)
{
    ...
    sems[prd_400Hz] = xSemaphoreCreateBinary();
    if (mKHz) {
    ...
    vTraceSetSemaphoreName(sems[prd_400Hz], "400Hz_Sem");
    if (mKHz) {
    ...
    xTaskCreate(period_task_400Hz, "400Hz",  PERIOD_TASKS_STACK_SIZE_BYTES/4, NULL, PRIORITY_CRITICAL + 4, NULL );
    if (mKHz) {
        xTaskCreate(period_task_1000Hz, "1000Hz", PERIOD_TASKS_STACK_SIZE_BYTES/4, NULL, PRIORITY_CRITICAL + 5, NULL);
    }
}
...
bool periodicSchedulerTask::run(void *p)
{
    ...
    if (mKHz) {
        if (handlePeriodicSemaphore(prd_1000Hz, 1)) {
        	if (handlePeriodicSemaphore(prd_400Hz, 2.5)){            // Run 400Hz every 2.5th time of  1000Hz
     ...
     if (handlePeriodicSemaphore(prd_400Hz, 1)) {
        if (handlePeriodicSemaphore(prd_100Hz, 4)) {        // Run 100Hz each time since we get unblocked at 10Hz
            if (handlePeriodicSemaphore(prd_10Hz, 40)) {    // Run 10Hz every 10th time of 100Hz
                if (handlePeriodicSemaphore(prd_1Hz, 40)) { // Run 1Hz every 10th time of 10Hz
                    ; // 1Hz task ran; nothing to do
                }
            }
        }
...
bool periodicSchedulerTask::handlePeriodicSemaphore(const uint8_t index, const uint8_t frequency)
{
    ...
    static const char * overrunMsg[] = { "1Hz task overrun", "10Hz task overrun", "100Hz task overrun", "400Hz task overrun", "1000Hz task overrun" };
